        ...
        if (Auth::user()) {
            if (Auth::user()->role == 1) {
                ...
                if (isset($request->image)) {
                    $imageName = rand() . $product->slug . '.' . $request->image->extension();
                    $request->image->move('backend/images/product/', $imageName);
                    $product->image = $imageName; // storing image name on database.
                }
                ...
                if (isset($request->galleryImage)) {
                    foreach ($request->galleryImage as $image) {
                        ...
                        $imageName = rand() . $product->slug . '.' . $image->extension();
                        $image->move('backend/images/galleryImage/', $imageName);
                        $galleryImage->image = $imageName; // storing image name on database.
            ...
            if (Auth::user()->role == 1) {
                ...
                if (isset($request->image)) {
                    if ($product->image && file_exists('backend/images/product' . $product->image)) {
                        unlink('backend/images/category/' . $product->image);
                    }
                    $imageName = rand() . $product->slug . '.' . $request->image->extension();
                    $request->image->move('backend/images/product/', $imageName);
                    $product->image = $imageName; // storing image name on database.
